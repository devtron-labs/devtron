...
func InitializeApp() (*App, error) {
	...
	clusterServiceImpl := cluster.NewClusterServiceImpl(clusterRepositoryImpl, environmentRepositoryImpl, grafanaClientImpl, sugaredLogger, installedAppRepositoryImpl, k8sUtil)
	environmentServiceImpl := cluster.NewEnvironmentServiceImpl(environmentRepositoryImpl, clusterServiceImpl, sugaredLogger, k8sUtil, propertiesConfigServiceImpl, grafanaClientImpl)
	...
	clusterServiceClientImpl := cluster2.NewServiceClientImpl(argoCDSettings, sugaredLogger)
	...
	userAuthRouterImpl, err := user2.NewUserAuthRouterImpl(sugaredLogger, userAuthHandlerImpl, argoCDSettings, userServiceImpl, dexConfig)
	if err != nil {
		return nil, err
	}
	...
	teamRestHandlerImpl := team2.NewTeamRestHandlerImpl(sugaredLogger, teamServiceImpl, userServiceImpl, enforcerImpl, validate, enforcerUtilImpl, userAuthServiceImpl)
	...
	userRouterImpl := user2.NewUserRouterImpl(userRestHandlerImpl, config2, argocdServerConfig, argoCDSettings)
	...
	ssoLoginServiceImpl := sso.NewSSOLoginServiceImpl(sugaredLogger, ssoLoginRepositoryImpl, k8sUtil, acdAuthConfig)
	...
	helmUserServiceImpl := user.NewHelmUserServiceImpl(sugaredLogger, helmUserRepositoryImpl, helmUserRoleRepositoryImpl, sessionManager)
	helmUserRestHandlerImpl := user2.NewHelmUserRestHandlerImpl(helmUserServiceImpl, validate, sugaredLogger)
