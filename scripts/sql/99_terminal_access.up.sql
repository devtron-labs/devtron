...
('terminal-access-pod','{"apiVersion":"v1","kind":"Pod","metadata":{"name":"${pod_name}"},"spec":{"serviceAccountName":"${pod_name}-sa","nodeSelector":{"kubernetes.io/hostname":"${node_name}"},"containers":[{"name":"internal-kubectl","image":"${base_image}","command":["/bin/sh","-c","--"],"args":["while true; do sleep 30; done;"]}]}}', now(), 1, now(), 1);