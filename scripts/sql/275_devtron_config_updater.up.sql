...
VALUES (nextval('id_seq_plugin_step_variable'), (SELECT ps.id FROM plugin_metadata p inner JOIN plugin_step ps on ps.plugin_id=p.id WHERE p.plugin_version='1.0.0' and p.name='Devtron Config Updater' and p.deleted=false and ps."index"=1 and ps.deleted=false), 'DashboardUrl','STRING','Dashboard url of Devtron for eg. https://previw.devtron.ai',true,false,'INPUT','NEW',1 ,'f','now()', 1, 'now()', 1,null),
...
(nextval('id_seq_plugin_step_variable'), (SELECT ps.id FROM plugin_metadata p inner JOIN plugin_step ps on ps.plugin_id=p.id WHERE p.plugin_version='1.0.0' and p.name='Devtron Config Updater' and p.deleted=false and ps."index"=1 and ps.deleted=false), 'FileNameToBeMounted','STRING','Name of the file to be mounted as volume. Default is same as FileName',true,true,'INPUT','NEW',1 ,'f','now()', 1, 'now()', 1, null),
...
(nextval('id_seq_plugin_step_variable'), (SELECT ps.id FROM plugin_metadata p inner JOIN plugin_step ps on ps.plugin_id=p.id WHERE p.plugin_version='1.0.0' and p.name='Devtron Config Updater' and p.deleted=false and ps."index"=1 and ps.deleted=false), 'ExecutionScript','STRING','Provide the script to create the config/secret file.',true,false,'INPUT','NEW',1 ,'f','now()', 1, 'now()', 1, null);
