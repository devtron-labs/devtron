...
VALUES (nextval('id_seq_plugin_step_variable'), (SELECT ps.id FROM plugin_metadata p inner JOIN plugin_step ps on ps.plugin_id=p.id WHERE p.name='Pull images from container repository' and ps."index"=1 and ps.deleted=false), 'REPOSITORY','STRING','Provide the repository name for polling, this will override the default repository used in build configuration',true,true,'INPUT','NEW',1 ,'f','now()', 1, 'now()', 1);
...
INSERT INTO "plugin_step_variable" ("id", "plugin_step_id", "name", "format", "description", "is_exposed", "allow_empty_value","variable_type", "value_type", "variable_step_index",reference_variable_name, "deleted", "created_on", "created_by", "updated_on", "updated_by")
VALUES (nextval('id_seq_plugin_step_variable'), (SELECT ps.id FROM plugin_metadata p inner JOIN plugin_step ps on ps.plugin_id=p.id WHERE p.name='Pull images from container repository' and ps."index"=1 and ps.deleted=false), 'DOCKER_REPOSITORY','STRING','Provide the repository name for polling.',false,true,'INPUT','GLOBAL',1 ,'DOCKER_REPOSITORY','f','now()', 1, 'now()', 1);
