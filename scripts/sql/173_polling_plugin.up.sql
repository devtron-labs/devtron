...
VALUES (nextval('id_seq_plugin_metadata'), 'Pull images from container repository','Polls a container repository and pulls images stored in the repository which can be used for deployment.','PRESET','https://raw.githubusercontent.com/devtron-labs/devtron/plugin-poll-support/assets/plugin-poll-container-registry.png','f', 'now()', 1, 'now()', 1);
...
VALUES (nextval('id_seq_plugin_pipeline_script'),'CONTAINER_IMAGE','t','quay.io/devtron/test:f7309681-1596-13074','f','now()',1,'now()',1);
...
VALUES (nextval('id_seq_plugin_step_variable'), (SELECT ps.id FROM plugin_metadata p inner JOIN plugin_step ps on ps.plugin_id=p.id WHERE p.name='Pull images from container repository' and ps."index"=1 and ps.deleted=false), 'REPOSITORY','STRING','Provide all the repositories with string seperated values for polling.',true,true,'INPUT','NEW',1 ,'f','now()', 1, 'now()', 1);
