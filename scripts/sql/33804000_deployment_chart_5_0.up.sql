...
SELECT 'deployment-chart_5-0-0', '5.0.0', 't', 't', 'now()', 1, 'now()', 1, 'Deployment', 'pipeline-values.yaml'
WHERE NOT EXISTS (
    SELECT 1 FROM "public"."chart_ref" WHERE "location" = 'deployment-chart_5-0-0'
);
...
SELECT
    ...
    (SELECT "id" FROM "public"."chart_ref" WHERE "location" = 'deployment-chart_5-0-0'),
...
WHERE NOT EXISTS (
    ...
    WHERE "global_strategy_metadata_id" = (SELECT "id" FROM "public"."global_strategy_metadata" WHERE "name" = 'ROLLING')
      AND "chart_ref_id" = (SELECT "id" FROM "public"."chart_ref" WHERE "location" = 'deployment-chart_5-0-0')
);
