...
test-integration:
	docker run --env-file=wireNilChecker.env  --privileged -d --name dind-test1 -v $(PWD)/:/wirenil/:ro -v $(PWD)/temp/:/tempfile docker:dind
	docker exec dind-test1 sh -c "mkdir test && cp -r wirenil/* test/ && ./test/tests/integrationTesting/exportEnvsExecuteWireNilChecker.sh"
run: build
	./devtron
.PHONY: build
