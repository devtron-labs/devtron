...
type AppStoreValuesRestHandlerImpl struct {
	...
	appStoreValuesService app_store.AppStoreValuesService
}

func NewAppStoreValuesRestHandlerImpl(Logger *zap.SugaredLogger, userAuthService user.UserService,
	appStoreValuesService app_store.AppStoreValuesService) *AppStoreValuesRestHandlerImpl {
...
func (handler AppStoreValuesRestHandlerImpl) CreateAppStoreVersionValues(w http.ResponseWriter, r *http.Request) {
	...
	var request app_store_bean.AppStoreVersionValuesDTO
...
func (handler AppStoreValuesRestHandlerImpl) FindValuesById(w http.ResponseWriter, r *http.Request) {
	...
	kind := vars["kind"]
	if len(kind) == 0 || (kind != app_store_bean.REFERENCE_TYPE_DEPLOYED && kind != app_store_bean.REFERENCE_TYPE_DEFAULT && kind != app_store_bean.REFERENCE_TYPE_TEMPLATE && kind != app_store_bean.REFERENCE_TYPE_EXISTING) {
...
func (handler AppStoreValuesRestHandlerImpl) FindValuesByAppStoreIdAndReferenceType(w http.ResponseWriter, r *http.Request) {
	...
	res, err := handler.appStoreValuesService.FindValuesByAppStoreIdAndReferenceType(appStoreVersionId, app_store_bean.REFERENCE_TYPE_TEMPLATE)
	if err != nil {
...
func (handler AppStoreValuesRestHandlerImpl) GetSelectedChartMetadata(w http.ResponseWriter, r *http.Request) {
	...
	decoder := json.NewDecoder(r.Body)
	var request app_store.ChartMetaDataRequestWrapper
	err = decoder.Decode(&request)
	if err != nil {
