...
type InstalledAppRestHandlerImpl struct {
	...
	installedAppService app_store.InstalledAppService
...
func NewInstalledAppRestHandlerImpl(Logger *zap.SugaredLogger, userAuthService user.UserService,
	enforcer casbin.Enforcer, enforcerUtil rbac.EnforcerUtil, installedAppService app_store.InstalledAppService,
...
func (handler InstalledAppRestHandlerImpl) CreateInstalledApp(w http.ResponseWriter, r *http.Request) {
	...
	var request app_store_bean.InstallAppVersionDTO
...
func (handler InstalledAppRestHandlerImpl) GetAllInstalledApp(w http.ResponseWriter, r *http.Request) {
	...
	filter := &app_store_bean.AppStoreFilter{OnlyDeprecated: onlyDeprecated, ChartRepoId: chartRepoIds, AppStoreName: appStoreName, EnvIds: envIds, AppName: appName, ClusterIds: clusterIds}
	if size > 0 {
		filter.Size = size
...
func (handler InstalledAppRestHandlerImpl) GetInstalledAppsByAppStoreId(w http.ResponseWriter, r *http.Request) {
	...
	var installedAppsResponse []app_store_bean.InstalledAppsResponse
	for _, app := range res {
		//rbac block starts from here
...
func (handler InstalledAppRestHandlerImpl) DeleteInstalledApp(w http.ResponseWriter, r *http.Request) {
	...
	//rback block ends here
	request := app_store_bean.InstallAppVersionDTO{}
...
func (handler *InstalledAppRestHandlerImpl) DeployBulk(w http.ResponseWriter, r *http.Request) {
	...
	decoder := json.NewDecoder(r.Body)
	var request app_store_bean.ChartGroupInstallRequest
	err = decoder.Decode(&request)
	if err != nil {
...
func (handler *InstalledAppRestHandlerImpl) CheckAppExists(w http.ResponseWriter, r *http.Request) {
	...
	decoder := json.NewDecoder(r.Body)
	var request []*app_store_bean.AppNames
	err = decoder.Decode(&request)
	if err != nil {
