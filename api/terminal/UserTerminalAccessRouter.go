...
func (router UserTerminalAccessRouterImpl) InitTerminalAccessRouter(userTerminalAccessRouter *mux.Router) {
	...
	userTerminalAccessRouter.Path("/get").
		HandlerFunc(router.userTerminalAccessRestHandler.FetchTerminalStatus).Queries("terminalAccessId", "{terminalAccessId}", "{namespace}", "shellName", "{shellName}").Methods("GET")
