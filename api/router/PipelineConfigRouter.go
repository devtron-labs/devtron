...
func (router PipelineConfigRouterImpl) initPipelineConfigRouter(configRouter *mux.Router) {
	...
	configRouter.Path("/cd-pipeline/patch/deployment").HandlerFunc(router.restHandler.ChangeCdAppType).Methods("POST")
	...
	configRouter.Path("/environment/{env-id}").HandlerFunc(router.restHandler.GetAppListByEnvId).Methods("GET")
