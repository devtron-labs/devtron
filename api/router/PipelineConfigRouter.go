...
func (router PipelineConfigRouterImpl) initPipelineConfigRouter(configRouter *mux.Router) {
	...
	configRouter.Path("/external-ci").HandlerFunc(router.restHandler.CreateWebhookCi).Methods("POST")
	...
	configRouter.Path("/externalCi-pipeline/trigger").HandlerFunc(router.restHandler.TriggerExternalCiPipeline).Methods("POST")
