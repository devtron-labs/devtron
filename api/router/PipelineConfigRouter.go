...
func (router PipelineConfigRouterImpl) initPipelineConfigRouter(configRouter *mux.Router) {
	...
	configRouter.Path("/deployments").Queries("appId", "{appId}").Queries("envId", "{envId}").HandlerFunc(router.restHandler.GetTemplateComparisonMetadata).Methods("GET")
	configRouter.Path("/data").HandlerFunc(router.restHandler.GetDeploymentTemplateData).Methods("POST")
