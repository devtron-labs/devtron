...
func (router PipelineConfigRouterImpl) initPipelineConfigRouter(configRouter *mux.Router) {
	...
	configRouter.Path("/image-tagging/{artifactId}").HandlerFunc(router.restHandler.CreateUpdateImageTagging).Methods("POST")
	configRouter.Path("/image-tagging/{artifactId}").HandlerFunc(router.restHandler.GetImageTaggingData).Methods("GET")
}
