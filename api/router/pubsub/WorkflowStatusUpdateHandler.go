...
func NewWorkflowStatusUpdateHandlerImpl(logger *zap.SugaredLogger, pubsubClient *pubsub1.PubSubClientServiceImpl, ciHandler pipeline.CiHandler, cdHandler pipeline.CdHandler,
	...
	streamConfig := &nats.StreamConfig{
		Name:     pubsub1.KUBEWATCH_STREAM,
		Subjects: pubsub1.GetStreamSubjects(pubsub1.KUBEWATCH_STREAM),
	}
	err := pubsub1.AddStream(workflowStatusUpdateHandlerImpl.pubsubClient.NatsClient.JetStrCtxt, streamConfig, pubsub1.KUBEWATCH_STREAM)
	if err != nil {
...
func (impl *WorkflowStatusUpdateHandlerImpl) Subscribe() error {
	_, err := impl.pubsubClient.NatsClient.JetStrCtxt.QueueSubscribe(pubsub1.WORKFLOW_STATUS_UPDATE_TOPIC, pubsub1.WORKFLOW_STATUS_UPDATE_GROUP, func(msg *nats.Msg) {
	...
	}, nats.Durable(pubsub1.WORKFLOW_STATUS_UPDATE_DURABLE), nats.DeliverLast(), nats.ManualAck(), nats.BindStream(pubsub1.KUBEWATCH_STREAM))
...
func (impl *WorkflowStatusUpdateHandlerImpl) SubscribeCD() error {
	_, err := impl.pubsubClient.NatsClient.JetStrCtxt.QueueSubscribe(pubsub1.CD_WORKFLOW_STATUS_UPDATE, pubsub1.CD_WORKFLOW_STATUS_UPDATE_GROUP, func(msg *nats.Msg) {
	...
	}, nats.Durable(pubsub1.CD_WORKFLOW_STATUS_UPDATE_DURABLE), nats.DeliverLast(), nats.ManualAck(), nats.BindStream(pubsub1.KUBEWATCH_STREAM))
