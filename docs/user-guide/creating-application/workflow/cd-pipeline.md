...
{% hint style="info" %}
An application can have one or more CD pipelines, each for a specific environment. Images from the CI pipeline can be deployed to different environments through separate CD pipelines.

To add a new CD pipeline, click the + sign and choose the environment.
{% endhint %}
...
If you want to run it within your application, check the [Execute tasks in application environment](#execute-in-application-environment) option. Otherwise, leave it unchecked to run it within the Devtron build cluster."
...
A deployment strategy is a method of updating, downgrading, or creating new versions of an application. Below are some deployment configuration-based strategies.
