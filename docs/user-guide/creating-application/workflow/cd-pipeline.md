...
![](../../../.gitbook/assets/ca-workflow-cd-add.png)
...
In order to configure advance CD option click on `Advance Options` at the bottom.
...
To Update a CD Pipeline, go to `App Configurations` section, Click on `Workflow editor` and then click on the CD Pipeline you want to Update.
