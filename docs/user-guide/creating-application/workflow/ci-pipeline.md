...
### 1. Source Type
In source type, select the type of mechanism you want to use for triggering the CI pipeline. As of now, three mechanisms are available to work with, which are branch-fixed, Pull Requests and Tags Creations.
### 2. Branch Name
...
To get Advance options, Click on the `Advanced Options` button, a new advanced window will appear.
...
Here, the name of the pipeline can be edited else it will pick the auto-generated name.

###  Pipeline Execution

Select the type of execution for the pipeline. `Automatic` or `Manual`.

###  Select Code Source

As mentioned above, `Source Type` and `Branch Name` can be edited in the `Advanced Options` as well.
