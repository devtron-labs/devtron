...
import (
	...
	http2 "github.com/devtron-labs/common-lib-private/utils/http"
...
type ClusterConfig struct {
	ClusterId                       int //devtron specific entity, to modify this when this library gets extracted at a common level for other microservices too
	ClusterName                     string
	Host                            string
	ProxyUrl                        string
	BearerToken                     string
...
func NewK8sUtil(logger *zap.SugaredLogger, runTimeConfig *client.RuntimeConfig,
	sshTunnelWrapperService SSHTunnelWrapperService) *K8sUtil {
...
func (impl K8sUtil) GetRestConfigByCluster(clusterConfig *ClusterConfig) (*restclient.Config, error) {
	...
	if clusterConfig.Host == DefaultClusterUrl && len(bearerToken) == 0 {
		...
		if clusterConfig.ToConnectWithSSHTunnel {
			...
			//overriding server url with localhost url where ssh tunnel is hosted
...
func (impl K8sUtil) GetHostUrlForSSHTunnelConfiguredCluster(clusterConfig *ClusterConfig) (string, error) {
...
func (impl K8sUtil) CleanupForClusterUsedForVerification(config *ClusterConfig) {
