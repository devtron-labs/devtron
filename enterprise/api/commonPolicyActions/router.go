...
func (r *CommonPolicyRouterImpl) InitCommonPolicyRouter(router *mux.Router) {
	router.Path(fmt.Sprintf("/{%s}/app-env/list", policyGovernance.PathVariablePolicyTypeVariable)).HandlerFunc(r.restHandler.ListAppEnvPolicies).
		Methods("POST")
	router.Path(fmt.Sprintf("/{%s}/bulk/apply", policyGovernance.PathVariablePolicyTypeVariable)).HandlerFunc(r.restHandler.ApplyPolicyToIdentifiers).
		Methods("POST")
}
