...
// TODO : env value for baseServerUrl can be dynamic as well, given via command line, might be we dont need setCookie param since all need it
func getRestyClient(setCookie bool) *resty.Client {
     ...
     if(setCookie) {
         client.SetCookie(&http.Cookie{Name:"argocd.token",
                 Value: returnAuthToken(),
                 Path: "/",
                 Domain: "https://demo1.devtron.info:32443"})
     }
     return client
}

//this function will read basicTestData.json file and get baseServerUrl, username and pwd depending on environment
func getBaseServerDetails() (string, string, string) {
    var baseServerUrl, loginUserName, loginUserPwd string

    //Put environment name here against which you want to run the test
    envToTest := "demo"
    //STEP 1 : Open the file
    testDataJsonFile, err := os.Open("../testdata/basicTestData.json")
    if (nil != err) {
        util.GetLogger().Errorw("Unable to open the file. Error occurred !!", "err", err)
    }
    util.GetLogger().Infow("Opened basicTestData.json file successfully !")
    ...
    json.Unmarshal([]byte(byteValue), &envList)

    for i := 0; i < len(envList.EnvList); i++ {
        if(envToTest == envList.EnvList[i].EnvironmentName) {
            baseServerUrl = envList.EnvList[i].BaseServerUrl
            loginUserName = envList.EnvList[i].LogInUserName
            loginUserPwd = envList.EnvList[i].LogInUserPwd
        }
    }
...
func returnAuthToken() string {
    ...
    if(handleError(err, "returnAuthToken")) {
        return ""
    }
