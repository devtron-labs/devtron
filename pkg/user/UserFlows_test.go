...
func BenchmarkCreateDefaultPoliciesForAllTypesV2(b *testing.B) {
	...
	defaultRbacPolicyRepo := repository2.NewDefaultRbacPolicyDataRepositoryImpl(logger, dbConnection)
	defaultRbacRoleRepo := repository2.NewDefaultRbacRoleDataRepositoryImpl(logger, dbConnection)
	defaultRbacCacheFactory := repository2.NewDefaultRbacDataCacheFactoryImpl(logger, defaultRbacPolicyRepo, defaultRbacRoleRepo)
