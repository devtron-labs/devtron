// Code generated by mockery v2.20.0. DO NOT EDIT.

package mocks

import (
	repository "github.com/devtron-labs/devtron/pkg/pipeline/history/repository"
	mock "github.com/stretchr/testify/mock"
)

// CiPipelineHistoryRepository is an autogenerated mock type for the CiPipelineHistoryRepository type
type CiPipelineHistoryRepository struct {
	mock.Mock
}

// Save provides a mock function with given fields: ciPipelineHistory
func (_m *CiPipelineHistoryRepository) Save(ciPipelineHistory *repository.CiPipelineHistory) error {
	ret := _m.Called(ciPipelineHistory)

	var r0 error
	if rf, ok := ret.Get(0).(func(*repository.CiPipelineHistory) error); ok {
		r0 = rf(ciPipelineHistory)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// SaveCiEnvMappingHistory provides a mock function with given fields: CiEnvMappingHistory
func (_m *CiPipelineHistoryRepository) SaveCiEnvMappingHistory(CiEnvMappingHistory *repository.CiEnvMappingHistory) error {
	ret := _m.Called(CiEnvMappingHistory)

	var r0 error
	if rf, ok := ret.Get(0).(func(*repository.CiEnvMappingHistory) error); ok {
		r0 = rf(CiEnvMappingHistory)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

type mockConstructorTestingTNewCiPipelineHistoryRepository interface {
	mock.TestingT
	Cleanup(func())
}

// NewCiPipelineHistoryRepository creates a new instance of CiPipelineHistoryRepository. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewCiPipelineHistoryRepository(t mockConstructorTestingTNewCiPipelineHistoryRepository) *CiPipelineHistoryRepository {
	mock := &CiPipelineHistoryRepository{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
