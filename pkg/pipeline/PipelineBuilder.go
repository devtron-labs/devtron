...
func (impl PipelineBuilderImpl) createCdPipeline(app *pipelineConfig.App, pipeline *bean.CDPipelineConfigObject, userID int32, ctx context.Context) (pipelineRes int, err error) {
	...
	values := []byte(`{"ConfigMaps":{"enabled":true,"maps":[{"data":{"key":"value"},"external":false,"mountPath":"","name":"cm-2","type":"environment"},{"data":{"js_test_app_cm1_key1":"js-test-app-cm1-value1"},"external":false,"mountPath":"","name":"js-test-app-cm1","type":"environment"}]},"ConfigSecrets":{"enabled":true,"secrets":[{"data":{"jstestkey_sec1":"ZGF0YQ=="},"external":false,"mountPath":"","name":"jstest-secret-1","type":"environment"}]},"ContainerPort":[{"name":"app","port":8080,"servicePort":80}],"EnvVariables":[],"GracePeriod":30,"LivenessProbe":{"Path":"","failureThreshold":3,"initialDelaySeconds":20,"periodSeconds":10,"port":8080,"successThreshold":1,"timeoutSeconds":5},"MaxSurge":1,"MaxUnavailable":0,"MinReadySeconds":60,"ReadinessProbe":{"Path":"","failureThreshold":3,"initialDelaySeconds":20,"periodSeconds":10,"port":8080,"successThreshold":1,"timeoutSeconds":6},"Spec":{"Affinity":{"Values":"nodes","key":""}},"affinity":{},"app":"64","appMetrics":true,"args":{"enabled":false,"value":[]},"autoscaling":{"MaxReplicas":2,"MinReplicas":1,"TargetCPUUtilizationPercentage":90,"TargetMemoryUtilizationPercentage":80,"enabled":false},"dbMigrationConfig":{"enabled":false},"deployment":{"strategy":{"blueGreen":{"autoPromotionEnabled":false,"autoPromotionSeconds":30,"previewReplicaCount":1,"scaleDownDelaySeconds":30},"rolling":{"maxSurge":"25%","maxUnavailable":3}}},"deploymentType":"ROLLING","env":"20","image":{"pullPolicy":"IfNotPresent"},"ingress":{"annotations":{},"enabled":true,"host":"","path":"","tls":[]},"ingressInternal":{"annotations":{},"enabled":false,"host":"","path":"","tls":[]},"nodeSelector":{},"orchestrator.deploymant.algo":1,"pauseForSecondsBeforeSwitchActive":30,"pipelineName":"js-test-app-cd-test","prometheus":{"release":"monitoring"},"releaseVersion":"4","replicaCount":1,"resources":{"limits":{"cpu":"0.06","memory":"20Mi"},"requests":{"cpu":"0.05","memory":"10Mi"}},"secret":{"data":{},"enabled":false},"server":{"deployment":{"enabled":true,"image":"086341552770.dkr.ecr.ap-southeast-1.amazonaws.com/orion-test","image_tag":"88ee85c6-130"}},"service":{"type":"ClusterIP"},"servicemonitor":{"enabled":false},"tolerations":[],"volumeMounts":[],"volumes":[],"waitForSecondsBeforeScalingDown":30}`)
	chartGitAttr := &util.ChartConfig{
		FileName:       fmt.Sprintf("_%d-values.yaml", envOverride.TargetEnvironment),
		FileContent:    string(values),
