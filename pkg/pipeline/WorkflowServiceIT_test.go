...
func TestWorkflowServiceImpl_SubmitWorkflow(t *testing.T) {
	...
	t.Run("Verify submit workflow with S3 archive logs", func(t *testing.T) {
		...
		if createdWf.Spec.Templates[0].Name == CI_WORKFLOW_NAME {
	...
	t.Run("Verify submit workflow for Jobs", func(t *testing.T) {
		...
		createdWf, _ := workflowServiceImpl.SubmitWorkflow(&workflowRequest, nil, nil, true)
...
func verifyTemplateSpecContainerEnv(t *testing.T, template v1alpha1.Template, workflowServiceImpl *WorkflowServiceImpl) {
	...
	for _, env := range template.Container.Env {
		if env.Name == "IMAGE_SCANNER_ENDPOINT" {
			assert.Equal(t, workflowServiceImpl.ciConfig.ImageScannerEndpoint, env.Value)
		}
	}
}
...
func verifyResourceLimitAndRequest(t *testing.T, template v1alpha1.Template, workflowServiceImpl *WorkflowServiceImpl) {
	...
	assert.Equal(t, resourceLimit["memory"], resource.MustParse(workflowServiceImpl.ciConfig.LimitMem))
	//assert.Equal(t, resourceRequest["cpu"], resource.MustParse(workflowServiceImpl.ciConfig.ReqCpu))
	assert.Equal(t, resourceRequest["memory"], resource.MustParse(workflowServiceImpl.ciConfig.ReqMem))
}
...
func verifyS3BlobStorage(t *testing.T, template v1alpha1.Template, workflowServiceImpl *WorkflowServiceImpl, workflowRequest WorkflowRequest) {
	...
	assert.Equal(t, workflowServiceImpl.ciConfig.DefaultBuildLogsKeyPrefix+"/"+workflowRequest.WorkflowNamePrefix, template.ArchiveLocation.S3.Key)
...
func verifyToleration(t *testing.T, workflowServiceImpl *WorkflowServiceImpl, createdWf *v1alpha1.Workflow) {
	...
	assert.Equal(t, workflowServiceImpl.ciConfig.TaintKey, createdWf.Spec.Tolerations[0].Key)
	assert.Equal(t, v12.TolerationOpEqual, createdWf.Spec.Tolerations[0].Operator)
	assert.Equal(t, workflowServiceImpl.ciConfig.TaintValue, createdWf.Spec.Tolerations[0].Value)
