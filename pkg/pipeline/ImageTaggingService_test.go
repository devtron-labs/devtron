...
func TestCreateOrUpdateImageTagging(t *testing.T) {
	...
	initRepos := func(fn string) (ImageTaggingService, *mocks.ImageTaggingRepository, *mocks2.CiPipelineRepository, *mocks2.PipelineRepository, *mocks3.EnvironmentRepository) {
	...
	t.Run("Valid Request, error in GetTagsByAppId", func(tt *testing.T) {
		...
		assert.Equal(tt, 0, len(res.ImageComment.Comment))
	...
	t.Run("tagNameValidation", func(tt *testing.T) {
		...
		tt.Run("invalid tag name 1", func(ttt *testing.T) {
			//invalid tag name 1
			testTagName := ".django"
			returnErr := tagNameValidation(testTagName)
		...
		tt.Run("invalid tag name 2", func(ttt *testing.T) {
			//invalid tag name 2
			testTagName := "-django"
			returnErr := tagNameValidation(testTagName)
			assert.NotNil(tt, returnErr)
		...
		tt.Run("invalid tag name 3", func(ttt *testing.T) {
			//invalid tag name 3
			testTagName := ""
			returnErr := tagNameValidation(testTagName)
			assert.NotNil(tt, returnErr)
		...
		tt.Run("invalid tag name 4", func(ttt *testing.T) {
			//invalid tag name 4
	...
	t.Run("GetTaggingDataMapByAppId", func(tt *testing.T) {
		...
		tt.Run("inValid test,GetImageCommentsByAppId throws error", func(ttt *testing.T) {
			...
			mockedImageTaggingRepo := mocks.NewImageTaggingRepository(tt)
			mockedImageTaggingRepo.On("GetTagsByAppId", appId).Return(testTags, nil)
			mockedImageTaggingRepo.On("GetImageCommentsByArtifactIds", []int{testArtifactId1, testArtifactId2}).Return(nil, errors.New(testErr))
			imageTaggingService := NewImageTaggingServiceImpl(mockedImageTaggingRepo, nil, nil, nil, sugaredLogger)
			resMap, err := imageTaggingService.GetTaggingDataMapByAppId(appId)
			assert.Nil(ttt, resMap)
			assert.NotNil(ttt, err)
