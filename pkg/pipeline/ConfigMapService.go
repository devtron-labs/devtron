...
func (impl ConfigMapServiceImpl) CSGlobalAddUpdate(configMapRequest *bean.ConfigDataRequest) (*bean.ConfigDataRequest, error) {
	...
	//VARIABLE_MAPPING_UPDATE
	data, err := bean.GetDecodedData(model.SecretData)
	if err != nil {
		return nil, err
	}
	err = impl.extractAndMapVariables(data, model.Id, repository5.SecretAppLevel, configMapRequest.UserId)
	if err != nil {
		return nil, err
	}
...
func (impl ConfigMapServiceImpl) CSEnvironmentAddUpdate(configMapRequest *bean.ConfigDataRequest) (*bean.ConfigDataRequest, error) {
	...
	err = impl.extractAndMapVariables(data, model.Id, repository5.SecretEnvLevel, configMapRequest.UserId)
	if err != nil {
		return nil, err
	}
...
func (impl ConfigMapServiceImpl) CSGlobalDelete(name string, id int, userId int32) (bool, error) {
	...
	if found {
		...
		err = impl.extractAndMapVariables(data, model.Id, repository5.SecretAppLevel, model.UpdatedBy)
		if err != nil {
			return false, err
		}
		_, err = impl.configMapRepository.UpdateAppLevel(model)
...
func (impl ConfigMapServiceImpl) CSEnvironmentDelete(name string, id int, userId int32) (bool, error) {
	...
	if found {
		...
		err = impl.extractAndMapVariables(data, model.Id, repository5.SecretEnvLevel, model.UpdatedBy)
		if err != nil {
			return false, err
		}
		_, err = impl.configMapRepository.UpdateEnvLevel(model)
