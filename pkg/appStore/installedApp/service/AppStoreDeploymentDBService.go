...
func (impl *AppStoreDeploymentServiceImpl) AppStoreDeployOperationDB(installRequest *appStoreBean.InstallAppVersionDTO, tx *pg.Tx,
	...
	if environment.IsVirtualEnvironment {
		if util.IsAcdApp(installRequest.DeploymentAppType) || util.IsHelmApp(installRequest.DeploymentAppType) {
	...
	if installRequest.DeploymentAppType == "" {
		installRequest.DeploymentAppType = util.PIPELINE_DEPLOYMENT_TYPE_HELM
		if environment.IsVirtualEnvironment {
			installRequest.DeploymentAppType = util.PIPELINE_DEPLOYMENT_TYPE_MANIFEST_DOWNLOAD
		} else if gitOpsConfigStatus.IsGitOpsConfigured && appInstallationMode == globalUtil.SERVER_MODE_FULL && !isOCIRepo {
			installRequest.DeploymentAppType = util.PIPELINE_DEPLOYMENT_TYPE_ACD
		}
	}
	...
	installRequest.HelmPackageName = adapter.GeneratedHelmPackageName(installRequest, installedApp.UpdatedOn)
