...
func (impl *CdWorkflowRepositoryImpl) GetLatestTriggersOfHelmPipelinesStuckInNonTerminalStatuses(deployedBeforeMinutes int) ([]*CdWorkflowRunner, error) {
	...
	err := impl.dbConnection.
		...
		Where("workflow_type=?", bean.CD_WORKFLOW_TYPE_DEPLOY).
		Where("started_on < NOW() - INTERVAL '? minutes'", deployedBeforeMinutes).
		Where("status not in (?)", pg.In([]string{WorkflowAborted, WorkflowFailed, WorkflowSucceeded, application.HIBERNATING, string(health.HealthStatusHealthy), string(health.HealthStatusDegraded)})).
		Where("cd_workflow_id in (select DISTINCT ON (pipeline_id) max(id) as id from cd_workflow group by pipeline_id, id order by pipeline_id, id desc)").
		Where("pipeline.deployment_app_type = ?", util.PIPELINE_DEPLOYMENT_TYPE_HELM).
