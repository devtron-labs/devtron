...
type PipelineRepository interface {
	...
	GetArgoPipelinesHavingLatestTriggerStuckInNonTerminalStatuses(deployedBeforeMinutes int, pipelineStatusCheckDeployedSince int) ([]*Pipeline, error)
...
func (impl PipelineRepositoryImpl) GetArgoPipelinesHavingLatestTriggerStuckInNonTerminalStatuses(deployedBeforeMinutes int, pipelineStatusCheckDeployedSince int) ([]*Pipeline, error) {
	var pipelines []*Pipeline
	queryString := `select p.* from pipeline p inner join cd_workflow cw on cw.pipeline_id = p.id  
    inner join cd_workflow_runner cwr on cwr.cd_workflow_id=cw.id  
	...
	_, err := impl.dbConnection.Query(&pipelines, queryString, deployedBeforeMinutes, pipelineStatusCheckDeployedSince,
