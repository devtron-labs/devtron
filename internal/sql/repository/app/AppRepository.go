...
func (repo AppRepositoryImpl) fixMultipleHelmAppsWithSameName(appName string) error {
	...
	installAppUpdateQuery := fmt.Sprintf("update installed_apps "+
		"set app_id=(select max(id) as id from app where app_name = '%s')"+
		" where"+
		" app_id in (select id from app where app_name='%s');", appName, appName)

	_, err := repo.dbConnection.Exec(installAppUpdateQuery)
	if err != nil {
