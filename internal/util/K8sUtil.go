...
func (impl K8sUtil) BuildK8sObjectListTableData(manifest *unstructured.UnstructuredList, namespaced bool, gvk schema.GroupVersionKind, validateResourceAccess func(namespace string, resourceIdentifier application.ResourceIdentifier) bool) (*ClusterResourceListMap, error) {
...
func (impl K8sUtil) ValidateResource(resourceObj map[string]interface{}, gvk schema.GroupVersionKind, validateCallback func(namespace string, resourceIdentifier application.ResourceIdentifier) bool) bool {
	...
	for _, resource := range resources {
		...
		allowed = allowed || validateCallback(namespace, resource)
	}
...
func (impl K8sUtil) getGVKAndNameForOwnerHierarchy(namespace string, owner interface{}) ([]application.ResourceIdentifier, error) {
	...
	if resourceReference["name"] != nil && resourceReference["name"] != "" {
		...
		case kube.ReplicaSetKind, kube.JobKind:
			...
			// if user doesn't have access to ReplicaSet with name firstname-lastname but has access to deployment with
